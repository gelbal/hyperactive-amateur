...
- [ ] **Step 2** — Type definitions in `src/types.ts`, `createInitialState()` factory + tests
- [ ] **Step 3** — Zustand store skeleton with `toggleStep`, `setBpm`, `setSwing`, `setTrackVolume`, `setTrackMuted` + tests
- [ ] **Step 4** — `<StepGrid />` renders 8×16 buttons wired to store, downbeats emphasized

## Phase 2 · Audio sequencer

- [ ] **Step 5** — Tone.js bootstrap, Transport ticks 16ths, metronome on every step
- [ ] **Step 6** — Triggers gated on `track.steps[i] === true`, mute respected, per-track pitches
...
- [ ] **Step 22** — `useKeyboardTriggers` hook for `1`–`8` (and Numpad), `triggerTrack` extracted
