# Hyperactive Amateur — implementation checklist
...
- [x] **Step 1** — Vite + React + TS scaffolding, Tailwind, Vitest, deps installed, blank `<h1>Hyperactive Amateur</h1>` page
...
- [x] **Step 25** — Debounced auto-save (500ms), rehydrate on mount, decode blobs back to `AudioBuffer`
...
- [x] **Step 31** — Swing slider, compatibility banner, empty states, keyboard hints, README, smoke-test checklist
...
- [x] **v1.1-5** — *Meaty:* video engine pendingTriggers + boundary scheduleRepeat + quantize-to-window
- [x] **v1.1-6** — `<CutSubdivisionSelect />` dropdown in top bar
...
- [x] **v1.1-10** — `aiAutoTag.ts` Gemini classification call with confidence
- [x] **v1.1-11** — Wire into recording flow; hat → audio-only auto-default; user-toggle bookkeeping

## Pattern variations (§ 3.5)

- [x] **v1.1-12** — `varyPattern()` (busier / fill / halftime / strip)
- [x] **v1.1-13** — Variation buttons next to Suggest with same Undo flow
