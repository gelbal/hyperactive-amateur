...
Done in the current implementation slice:

- Phase 0 baseline and safety log were created in `docs/quality-pass/status.md`.
- The step-count contract decision was recorded: bar-aligned loop lengths in increments of 4 remain the intended contract.
- `api/gemini.ts` now enforces a model allowlist, narrow request validation, serialized body cap, Origin/Referer allowlist, stable upstream error mapping, and production fail-closed durable rate limiting.
- The Gemini proxy now auto-allows Vercel deployment origins from `VERCEL_PROJECT_PRODUCTION_URL`, `VERCEL_BRANCH_URL`, and `VERCEL_URL`; custom/public domains are still documented through `GEMINI_ALLOWED_ORIGINS`.
- The rate limiter now runs before the request body is read, covers token issuance and invalid-token attempts, uses route-scoped buckets, requires HTTPS limiter URLs in production, and uses Vercel's `x-vercel-forwarded-for` header for production client identity when present; plain `x-forwarded-for` is only a development fallback.
- Regression tests now cover current app request shapes, upstream SDK-to-REST mapping, production origin handling, body-cap/rate-limit ordering, validation guardrails, and the real Upstash/Vercel-KV REST limiter path.
- Production AI proxy environment variables are documented in `.env.example` and `README.md`.
- `.serena/` is ignored as a local tool artifact.
- Phase 1.2 dependency advisory cleanup is complete: `@vitest/ui` was removed as a direct dependency and the stale `test:ui` script was removed, the Vite/Vitest/React-plugin cluster was upgraded to Vite 8.1.0, Vitest 4.1.9, and `@vitejs/plugin-react` 6.0.3, transitive advisory packages were patched by `npm audit fix`, and `npm audit --audit-level=moderate` now reports zero vulnerabilities.
- Test/build compatibility with the new toolchain was restored by making Tone constructor mocks constructable under Vitest 4 and making the JSDOM `URL.createObjectURL` shim deterministic even when Node exposes its own stricter implementation.

Current status:

- Phase 1 is complete for this pass: the Gemini relay is protected by origin allowlisting, production Fetch Metadata checks, short-lived server-signed request tokens, route-scoped durable production rate limiting, and HTTPS-only production limiter URLs; dependency advisories are resolved.
- Phase 2 is complete for this pass: media lifecycle ownership, export abort behavior, recording/playback/export gating, and export-time mutation freezes have been implemented and adversarially re-reviewed.
- Phase 3 is complete for this pass: persisted projects now have schema validation/recovery, degraded-load backups and visible warnings, autosave flushes dirty recording edits after idle, autosave errors are logged, and step-count removal preserves the bar-aligned multiple-of-4 contract.
- Phase 4 is complete for this pass: visual trim-end behavior, AI overwrite-race prevention, service-worker asset caching, and reload-safe cache key handling are implemented and covered.
- Phase 5 is complete for this pass: React warning cleanup, console guardrails, and real-browser smoke coverage are implemented.

Remaining architectural follow-up:

- The Gemini proxy still accepts the current client-built Gemini request shapes after validation. It is no longer an open public relay, but the next hardening step is to replace the client-built prompt/schema contract with operation-specific server endpoints (`suggest`, `vary`, `autotag`, `batchAutotag`) that construct prompts and schemas server-side from bounded fields.
...
Current status: complete for this pass. The proxy hardening, production Fetch Metadata checks, short-lived server-signed request tokens, durable limiter, production deploy documentation, and regression coverage listed above are implemented. Browser cross-origin scripts cannot mint or spend tokens merely by setting `Origin`; durable rate limiting remains the abuse control for non-browser traffic.
...
Current status: complete for this pass. `npm audit --audit-level=moderate` reports zero vulnerabilities after removing the unused Vitest UI entry point, patching non-breaking transitive advisories, and upgrading the Vite/Vitest toolchain cluster. The Node engine requirement is now `^20.19.0 || >=22.12.0` to match the upgraded Vite/plugin requirements.
...
Current status: complete for this pass. Media stream release/suspend is centralized through `streamLifecycle.ts`; export renders have an exclusive abortable session; hidden-page export abort still suspends held preview/recording media; playback, pad, keyboard, recording, and export starts share one audible-action gate; export-owned transport cannot be stopped by Space/toggle; output-affecting project mutations, including tags and AI feel controls, no-op while export is active.
...
Current status: complete for this pass. Rehydrate now normalizes legacy and malformed persisted projects through a schema-versioned recovery path, writes a bounded backup before hydrating degraded data, exposes recovery warnings in the UI, and refuses to hydrate/start autosave if the backup write fails. Autosave now preserves dirty project edits made during recording and flushes them when recording returns to idle. Step removal now removes four-step blocks so loop lengths remain bar-aligned.
...
Status: done. `drawCurrentFrame` now stops drawing once the displayed
event reaches the clip trim end, clears the viewport, and repeated
triggers still restart from trim start.
...
Status: done. Pending Suggest and Variation requests now capture a
store-level project revision plus step count and apply only through a
guarded store action when the active grid is still current.
...
Status: done. The worker keeps `/api/` network-only, deletes old
versioned caches, runtime-caches same-origin assets as a fallback, and
production builds inject emitted `/assets/` files into the install-time
precache so first-load offline hard reloads can boot.
...
Status: done. The Vitest setup now fails React `act(...)` warnings
and unexpected non-`[HA]` `console.error` output. Component tests that
mutate subscribed state or resolve async handlers now wrap those paths
in `act`.
...
Status: done. `npm run smoke:browser` builds the app, serves the
production preview, and runs Playwright smoke coverage for app boot,
service-worker offline reload, keyboard gating, hidden-page media
suspension, and export error UI with mocked browser media APIs.
